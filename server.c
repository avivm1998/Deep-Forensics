	...
	int main(int argc,char** argv)
	{
        ...
        while(1) {
            if((read_bytes = recv(client_fd, buffer, MAX_PAYLOAD, 0)) == 0)
                break;
            ...
            strncpy(buffer, net_res, strlen(net_res));

            send(client_fd, &request.starting_address, sizeof(request.starting_address), 0);
            send(client_fd, &request.length, sizeof(request.length), 0);
            send(client_fd, buffer, MAX_PAYLOAD, 0);
        }
